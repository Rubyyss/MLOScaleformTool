"""
Curve processor for GTA V Scaleform Minimap Calculator addon.
...
class CurveProcessor:
    ...
    def _process_bezier_spline(spline, matrix, spline_points, all_points):
        ...
        for point in bezier_points:
            co, hl, hr = [matrix @ v for v in (point.co, point.handle_left, point.handle_right)]
    ...
    @staticmethod
    def normalize_curves(curve_data: Dict[str, Any], center_at_origin: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        for i, curve_obj in enumerate(curve_data.get("curves", [])):
            ...
            for spline in curve_obj:
                ...
                for seg in spline:
                    normalized_segments.append({
                        "type": seg["type"],
                        "points": [GPointF(pt.x - offset_x, pt.y - offset_y) for pt in seg["points"]]
                    })
            ...
            normalized_data.append({
                "splines": obj_normalized_splines,
                "style": curve_info
            })
    ...
    @staticmethod
    def simplify_curves(curve_data: Dict[str, Any], tolerance: float = 0.1) -> Dict[str, Any]:
        """
                    ...
                    if seg["type"] == "M":
                        ...
                        if points_to_simplify:
                            CurveProcessor._add_simplified_points(
                                points_to_simplify, simplified_spline, tolerance, 
    ...
    def _add_simplified_points(points, simplified_spline, tolerance, simplify_func):
        ...
        for i in range(1, len(simplified)):
            simplified_spline.append({
                "type": "L",
                "points": [simplified[i]]
            })
    
    @staticmethod
    def get_selected_curves(context: bpy.types.Context) -> Dict[str, Any]:
        """
        ...
        for obj in selected_objects:
            ...
            for spline in obj.data.splines:
                ...
                if spline.type == "BEZIER":
                    CurveProcessor._process_bezier_spline(spline, matrix, spline_points, object_points)
                else:
                    CurveProcessor._process_poly_spline(spline, matrix, spline_points, object_points)
