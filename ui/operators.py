"""
Operator definitions for GTA V Scaleform Minimap Calculator.
...
from ..utils.helpers import copy_to_clipboard, apply_fill_preset, apply_stroke_settings
...
class SCALEFORM_OT_calculate_dimensions(Operator):
    ...
    def execute(self, context):
        ...
        world_bounds = (
            WORLD_BOUNDS
            if settings.minimap_preset == "DEFAULT"
            else (
                settings.custom_world_min_x,
                settings.custom_world_max_x,
                settings.custom_world_min_y,
                settings.custom_world_max_y,
            )
        )
        ...
        scaleform_center = calculator.blender_to_scaleform(
            center_blender,
            settings.svg_scale,
            dimensions["width_svg"],
            dimensions["height_svg"],
        )
...
class SCALEFORM_OT_export_svg(Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        normalized_curves = CurveProcessor.normalize_curves(
            curve_data, settings.center_at_origin
        )

        # Get dimensions from scene
        dimensions = {
            key: getattr(scene, f"scaleform_{key}")
            for key in ["width_orig", "height_orig", "width_svg", "height_svg"]
        }
        ...
        svg_content = SVGExporter.generate_svg_content(
            dimensions, normalized_curves, settings, minimap_coords
        )
        ...
        # Report success to the user
        curve_count = (
            len(curve_data.get("curves_info", [])) if "curves_info" in curve_data else 0
        )
        self.report(
            {"INFO"},
            f"Exported {curve_count} curves to SVG at {self.filepath}\nScaleform data saved to {json_filepath}",
        )
        return {"FINISHED"}
...
class SCALEFORM_OT_copy_to_clipboard(Operator):
    ...
    def execute(self, context):
        ...
        value_to_copy = (
            self.value.replace(".", ",") if settings.use_comma_separator else self.value
        )
...
class SCALEFORM_OT_apply_fill_settings(Operator):
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"},
            f"Fill and stroke settings applied to {len(selected_curves)} curve objects",
        )
        return {"FINISHED"}
