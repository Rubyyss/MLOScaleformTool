"""
Panel definitions for GTA V Scaleform Minimap Calculator.
...
class SCALEFORM_PT_main_panel(Panel):
    """
    Main panel for the GTA V Scaleform Minimap Calculator.
    ...
    def draw(self, context):
        ...
        if scene.scaleform_has_valid_data:
            ...
            row = dim_box.row()
            row.label(
                text=f"Units: {scene.scaleform_width_orig:.2f} x {scene.scaleform_height_orig:.2f}"
            )
            row = dim_box.row()
            row.label(
                text=f"SVG: {scene.scaleform_width_svg * settings.svg_scale:.2f} x {scene.scaleform_height_svg * settings.svg_scale:.2f}"
            )
            ...
            if hasattr(scene, "scaleform_selected_curve_count"):
                row = dim_box.row()
                row.label(
                    text=f"Curves in calculation: {scene.scaleform_selected_curve_count}"
                )
            ...
            copy_op = row.operator(
                "scaleform.copy_to_clipboard", text="", icon="COPYDOWN"
            )
            copy_op.value = val_x
            ...
            if (
                hasattr(scene, "scaleform_position_scaleform_x")
                and scene.scaleform_position_scaleform_x != 0.0
            ):
    ...
    def draw(self, context):
        ...
        if selected_curves:
            ...
            if len(selected_curves) > 1:
                configured_curves = [
                    obj for obj in selected_curves if "scaleform_fill_preset" in obj
                ]
                row = info_box.row()
                row.label(
                    text=f"{len(configured_curves)}/{len(selected_curves)} curves have settings"
                )

            # If only one curve is selected, show its settings
            if (
                len(selected_curves) == 1
                and "scaleform_fill_preset" in selected_curves[0]
            ):
                ...
                row = info_box.row()
                r, g, b = (
                    obj.get("scaleform_fill_color_r", 0),
                    obj.get("scaleform_fill_color_g", 0),
                    obj.get("scaleform_fill_color_b", 0),
                )
                row.label(
                    text=f"Fill: #{int(r*255):02x}{int(g*255):02x}{int(b*255):02x}"
                )
                r, g, b = (
                    obj.get("scaleform_stroke_color_r", 0),
                    obj.get("scaleform_stroke_color_g", 0),
                    obj.get("scaleform_stroke_color_b", 0),
                )
                row = info_box.row()
                row.label(
